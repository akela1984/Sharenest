...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['join_location'])) {
        joinLocation($conn, $user['id'], $location_name);
    } elseif (isset($_POST['leave_location'])) {
        leaveLocation($conn, $user['id'], $location_id);
    }
}
...
function joinLocation($conn, $user_id, $location_name) {
    global $error;
...
function leaveLocation($conn, $user_id, $location_id) {
    global $error;
    ...
    if ($result->num_rows > 0) {
        ...
        deleteLocationImages($conn, $user_id, $location_id);
        deleteUserListings($conn, $user_id, $location_id);
        deleteUserLocation($conn, $user_id, $location_id);
    }
}

function deleteLocationImages($conn, $user_id, $location_id) {
    global $error;
    ...
    $stmt_images->bind_param("ii", $user_id, $location_id);
    $stmt_images->execute();
    ...
    $stmt_delete_images->bind_param("ii", $user_id, $location_id);
    $stmt_delete_images->execute();
}

function deleteUserListings($conn, $user_id, $location_id) {
    global $error;
    ...
    $stmt_delete_listings->bind_param("ii", $user_id, $location_id);
    $stmt_delete_listings->execute();
}

function deleteUserLocation($conn, $user_id, $location_id) {
    global $error;
    ...
    $stmt_delete_user_location->bind_param("ii", $user_id, $location_id);
...
<script>
    function confirmLeave() {
        return confirm('Are you sure you want to leave this location? All your current listings for this city will be permanently deleted and cannot be restored.');
    }
</script>
</head>
<body class="bg-light">
...
<div id="content" class="container mt-5">
    ...
    <h4 class="mb-3">Already Member Of:</h4>
        ...
        <?php while ($row = $locationsResult->fetch_assoc()) { 
            // Fetch additional info for each location
            $location_id = $row['location_id'];
        ...
        ?>
            <li class="list-group-item">
                <div class="top-row">
                    <span class="location-name"><?php echo htmlspecialchars($row['location_name']); ?></span>
                ...
                <div class="location-info mt-2">
                    <div class="details">
                        <p>Total Listings: <?php echo $info['total_listings']; ?></p>
                        <p>Last Item Added: <?php echo $last_item ? htmlspecialchars($last_item['title']) . " on " . htmlspecialchars($last_item['time_added']) : "No items added yet."; ?></p>
                    </div>
                    <div class="info-message">
                        <?php if ($info['total_listings'] == 0) { ?>
                            <div class="alert alert-info mt-2">
                                <a href="create_listing.php">Be the first to share unwanted items and promote sustainability!</a>
                            </div>
                        <?php } ?>
                    </div>
                </div>
...
<script>
    ...
    const username = '<?php echo $config['api']['geonames_username']; ?>'; // Fetch username from config
                ...
                if (data.geonames.length === 0) {
                    ...
                    data.geonames.forEach(place => {
                        if (!uniquePlaces.has(place.name)) {
                            ...
                            li.innerHTML = `<div class="top-row">
                                            ...
                                            <div class="location-info mt-2" data-location="${place.name}">
                                                <!-- This will be filled with JS -->
                                            </div>`;
                            cityList.appendChild(li);

                            // Fetch the location info with AJAX
                            fetchLocationInfo(place.name);
                        }
                    });
                }
            })
    ...
    function fetchLocationInfo(locationName) {
        fetch('location_info.php?location_name=' + encodeURIComponent(locationName))
            .then(response => response.json())
            .then(data => {
                const locationInfoDiv = document.querySelector(`.location-info[data-location="${locationName}"]`);
                if (locationInfoDiv) {
                    locationInfoDiv.innerHTML = `
                        <div class="details">
                            <p>Total Listings: ${data.total_listings}</p>
                            <p>Last Item Added: ${data.last_item ? `${data.last_item.title} on ${data.last_item.time_added}` : 'No items added yet.'}</p>
                        </div>
                        <div class="info-message">
                            ${data.total_listings == 0 ? '<div class="alert alert-info mt-2">Join this location and be the first to share unwanted items and promote sustainability!</div>' : ''}
                        </div>
                    `;
                }
            })
