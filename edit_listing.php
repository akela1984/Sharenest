            ...
            if ($stmt_update->execute()) {
                ...
                if (!empty($_FILES['images']['name'][0])) {
                    ...
                    if ($stmt_images === false) {
                        ...
                        $upload_errors = []; // Array to capture errors
                        ...
                        foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
                            ...
                            if (!move_uploaded_file($tmp_name, $file_path)) {
                                $upload_errors[] = "Failed to upload file: {$_FILES['images']['name'][$key]}";
                            } else {
                                ...
                                if ($stmt_insert_image === false) {
                                    $upload_errors[] = 'Prepare failed: ' . htmlspecialchars($conn->error);
                                } else {
                                    $stmt_insert_image->bind_param("is", $listing_id, $file_path);
                                    if (!$stmt_insert_image->execute()) {
                                        $upload_errors[] = "Failed to insert image path to database for file: {$file_name}";
                                    }
                                }
                            }
                        }
                        ...
                        if (!empty($upload_errors)) {
                            ...
                            $success_message .= " All images uploaded successfully.";
                        }
                    }
                }
            } else {
